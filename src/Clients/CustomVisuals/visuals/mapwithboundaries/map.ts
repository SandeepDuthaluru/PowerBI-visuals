    ...
    export class CustomVisual implements IVisual {        
        ...
        public init(options: VisualInitOptions): void {
			...
			setTimeout(() => {
                ...
                Microsoft.Maps.Events.addHandler(map, 'tiledownloadcomplete', () => this.loadStates(options.element.get(0)));
            });
        ...
        public loadStates(element) {
            d3MapTools = new D3OverlayManager(map);
						if(stateLayer != null)
                        {                            
                            if(stateLayer.svg[0][0].parentElement != null)
                            {
                            stateLayer.svg[0][0].parentElement.remove();                       }
                        }
            stateLayer = d3MapTools.addLayer({
                loaded: function (svg, projection) {
					//svg = this.svg = d3.select(element).append('svg');
					debugger;
                    var jsondata = "";
                    var xhr = new XMLHttpRequest();
					xhr.open('GET', encodeURI('https://icrisat.blob.core.windows.net/json/GeoJson.js'), false);
					xhr.onload = function() {
					if (xhr.status === 200) {
        ...
        public update(options: VisualUpdateOptions) {
            ...
            */
            ...
            var viewModel = CustomVisual.converter(dataViews[0], this.colorPalette);
